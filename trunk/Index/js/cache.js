...
$.cache={
	...
	buildTreeData : function(data){
		...
		for(var i=0,size=data.length;i<size;i++){
			...
			cid=item.cid;
			if(map[cid]){
				item.children=map[cid].children;
			}
			if(item.name){
